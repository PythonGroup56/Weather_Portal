...
class LoginTest(TestModel):
    ...
    def test_login_success(self):
        ...
        response=self.client.post(self.login_url, {'username': user.username, 'password': self.user['password1']})
        self.assertEqual(response.status_code, 302)
    
    def test_no_login_with_no_username(self):
        user = User.objects.create_user(username=self.user['username'], email=self.user['email'], 
        password=self.user['password1'])
        response=self.client.post(self.login_url, {'password': self.user['password1'], 'username':'invalid'})
        self.assertEqual(response.status_code, 200)
    
    def test_no_login_with_no_password(self):
        user = User.objects.create_user(username=self.user['username'], email=self.user['email'], 
        password=self.user['password1'])
        response=self.client.post(self.login_url, {'username': user.username, 'password': 'invalid'})
        self.assertEqual(response.status_code, 200)
